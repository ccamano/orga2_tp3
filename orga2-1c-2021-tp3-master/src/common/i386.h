...
LS_INLINE uint32_t rds(void) {
  uint32_t s;
  __asm __volatile("movl %%ds,%0" : "=r"(s));
  return s;
}

LS_INLINE uint32_t res(void) {
  uint32_t s;
  __asm __volatile("movl %%es,%0" : "=r"(s));
  return s;
}

LS_INLINE uint32_t rfs(void) {
  uint32_t s;
  __asm __volatile("movl %%fs,%0" : "=r"(s));
  return s;
}

LS_INLINE uint32_t rgs(void) {
  uint32_t s;
  __asm __volatile("movl %%gs,%0" : "=r"(s));
  return s;
}

LS_INLINE uint32_t rss(void) {
  uint32_t s;
  __asm __volatile("movl %%ss,%0" : "=r"(s));
  return s;
}
